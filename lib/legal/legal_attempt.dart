...
const chat_gpt_api="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"; // put api here if it expires
...
class _legal_pageState extends State<legal_page> {
  ...
  Widget build(BuildContext context) {
    return Container(
       ...
       child: DashChat(currentUser: currUser,        // who is currently sending the message
         onSend: (ChatMessage message_typed){        // when message 'm' is sent
...
Future<void> getChatResponse(ChatMessage message_typed) async{
    ...
    List<Messages> messageHistory= all_messages.reversed.map((message_typed){
       if(message_typed.user==currUser){
         return Messages(role: Role.user,content: message_typed.text);
       }
       else{
    ...
    final req=ChatCompleteText(model: GptTurbo0301ChatModel(), messages: messageHistory,maxToken: 100,);
    ...
    for(var element in response!.choices){
      if(element.message!=null){
       ...
       setState(() {
         all_messages.insert(0, ChatMessage(user: gptUser, createdAt: DateTime.now(),text: element.message!.content),);
       });
      }
    }
}
